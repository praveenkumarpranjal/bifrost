...
//     validating an OAuth config template). On success → /workspace/mcp-sessions/oauth-callback
...
func (h *OAuthHandler) handleOAuthCallback(ctx *fasthttp.RequestCtx) {
	...
	if err := h.oauthProvider.CompleteOAuthFlow(ctx, state, code); err != nil {
		...
		ctx.Redirect("/workspace/mcp-sessions/oauth-callback?status=failed&error="+url.QueryEscape("OAuth authentication failed."), fasthttp.StatusFound)
		return
	}
	ctx.Redirect("/workspace/mcp-sessions/oauth-callback?status=success", fasthttp.StatusFound)
}
...
//   - admin-test popup flow → /workspace/mcp-sessions/oauth-callback (posts a
...
func (h *OAuthHandler) handleCallbackError(ctx *fasthttp.RequestCtx, state, errorParam, errorDescription string) {
	...
	if isAdminTestFlow {
		ctx.Redirect("/workspace/mcp-sessions/oauth-callback?status=failed&error="+url.QueryEscape(userMsg), fasthttp.StatusFound)
		return
	}
