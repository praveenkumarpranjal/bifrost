...
func (f *fakeStore) CreateTempToken(_ context.Context, tok *tables.TempToken) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStore) DeleteTempTokensByResourceID(_ context.Context, scope, resourceID string) (int64, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	var deleted int64
...
func (f *fakeStore) DeleteExpiredTempTokens(_ context.Context, _ time.Time) (int64, error) {
	return 0, nil
}
